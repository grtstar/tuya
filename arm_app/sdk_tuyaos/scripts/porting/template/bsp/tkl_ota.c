...
OPERATE_RET tkl_ota_get_ability(UINT_T *image_size, TUYA_OTA_TYPE_E *type)
{
    ...
    typedef OPERATE_RET (*TYPE_T)(UINT_T *image_size, TUYA_OTA_TYPE_E *type);

    OPERATE_RET ret = OPRT_NOT_SUPPORTED;
    TYPE_T fun;

    fun = (TYPE_T)tkl_adapt_func_get(FUNC_NAME(tkl_ota_get_ability));

    if(fun) {
        ret = fun(image_size, type);
    }
...
{
    // --- BEGIN: user implements ---
    typedef OPERATE_RET (*TYPE_T)(UINT_T image_size, TUYA_OTA_TYPE_E type, TUYA_OTA_PATH_E path);

    OPERATE_RET ret = OPRT_NOT_SUPPORTED;
    TYPE_T fun;

    fun = (TYPE_T)tkl_adapt_func_get(FUNC_NAME(tkl_ota_start_notify));

    if(fun) {
        ret = fun(image_size, type, path);
    }
...
{
    // --- BEGIN: user implements ---
    typedef OPERATE_RET (*TYPE_T)(TUYA_OTA_DATA_T *pack, UINT_T* remain_len);

    OPERATE_RET ret = OPRT_NOT_SUPPORTED;
    TYPE_T fun;

    fun = (TYPE_T)tkl_adapt_func_get(FUNC_NAME(tkl_ota_data_process));

    if(fun) {
        ret = fun(pack, remain_len);
    }
...
{
    // --- BEGIN: user implements ---
    typedef OPERATE_RET (*TYPE_T)(BOOL_T reset);

    OPERATE_RET ret = OPRT_NOT_SUPPORTED;
    TYPE_T fun;

    fun = (TYPE_T)tkl_adapt_func_get(FUNC_NAME(tkl_ota_end_notify));
...
{
    // --- BEGIN: user implements ---
    typedef OPERATE_RET (*TYPE_T)(TUYA_OTA_FIRMWARE_INFO_T **info);

    OPERATE_RET ret = OPRT_NOT_SUPPORTED;
    TYPE_T fun;

    fun = (TYPE_T)tkl_adapt_func_get(FUNC_NAME(tkl_ota_get_old_firmware_info));
