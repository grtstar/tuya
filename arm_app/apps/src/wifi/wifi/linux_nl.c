...
static int read_mon_if_from_conf(void)
{
	...
	if (f != NULL) {
		...
		if (ret == file_size) {
			LOGDEBUG("read wifi monitor interface from %s\n", MON_CONF_PATH);
			...
			if (pos != NULL) {
				...
				if (len != 0 && len < MON_IF_SIZE) {
					...
					LOGDEBUG("monitor interface is '%s'\n", monitor_nl_inf_object.monitor_if);
					ret = 0;
				} else {
					LOGW("monitor interface name maybe error\n");
				}
			}
		}
...
static void *mon_recv_thread(void *p)
{
	...
	if (setsockopt(sockfd, SOL_SOCKET, SO_BINDTODEVICE, monitor_nl_inf_object.monitor_if, IFNAMSIZ-1) == -1) {
		LOGE("setsocket SO_BINDTODEVICE - %s\n", strerror(errno));
		close(sockfd);
		exit(EXIT_FAILURE);
	}
	for (;;) {
	...
	//free(msg.data.frame);
	//pthread_exit(NULL);
	//LOGD("monitor data handle thread exit\n");
}
...
static wmg_status_t linux_monitor_nl_set_channel(uint8_t channel)
{
	...
	if (monitor_nl_inf_object.monitor_nl_init_flag == WMG_FALSE) {
		LOGE("wifi monitor is not initialized\n");
		return WMG_STATUS_FAIL;
	}

	if (monitor_nl_inf_object.monitor_enable == WMG_FALSE) {
		LOGE("set monitor channel failed because wifi monitor is disabled\n");
		return WMG_STATUS_FAIL;
	}
	...
	if (ret < 0) {
		LOGE("failed to send netlink message for set channel of monitor\n");
		ret = WMG_STATUS_FAIL;
	...
	LOGDEBUG("wifi monitor set channel(%d) success\n", channel);
...
static int linux_monitor_nl_init(nl_data_frame_cb_t nl_data_frame_cb)
{
	wmg_status_t ret;
	if(monitor_nl_inf_object.monitor_nl_init_flag == WMG_FALSE) {
		LOGI("linux monitor nl init now\n");
		...
		if (monitor_nl_inf_object.nl_state != NULL) {
			...
			if (!ret) {
				LOGDEBUG("nl80211 init success\n");
			} else {
				LOGE("failed to init nl80211\n");
				return WMG_STATUS_FAIL;
			}
		}
		...
		LOGI("linux monitor nl already init\n");
	}
...
static int linux_monitor_nl_deinit(void)
{
	if(monitor_nl_inf_object.monitor_nl_init_flag == WMG_TRUE){
		LOGI("linux monitor nl deinit now\n");
		...
		LOGI("linux monitor nl already deinit\n");
	}
	return 0;
}
...
static int linux_monitor_nl_enable(uint8_t channel)
{
	...
	if (ret) {
		LOGE("failed to create linux nl dnata handle thread\n");
		return WMG_STATUS_FAIL;
	}
	...
	ret = nl_send_auto_complete(monitor_nl_inf_object.nl_state->nl_sock, msg);
	if (ret < 0) {
		LOGE("failed to send netlink message for enable wifi monitor\n");
		ret = WMG_STATUS_FAIL;
		goto nla_put_failure;
	} else {
		ret = WMG_STATUS_SUCCESS;
	...
	LOGDEBUG("wifi monitor enable success\n");
...
static int linux_monitor_nl_disable()
{
	...
	LOGDEBUG("wifi monitor disableing...\n");
	...
	ret = nl_send_auto_complete(monitor_nl_inf_object.nl_state->nl_sock, msg);
	if (ret < 0) {
		LOGE("failed to send netlink message for disable wifi monitor\n");
		ret = WMG_STATUS_FAIL;
		goto nla_put_failure;
	} else {
		ret = WMG_STATUS_SUCCESS;
	...
	LOGDEBUG("wifi monitor disable success\n");
...
static int linux_connect_to_nl()
{
	...
	if (!ret) {
		LOGDEBUG("get monitor interface success\n");
	} else {
		LOGW("get monitor interface failed, just use default 'wlan0'\n");
		strncpy(monitor_nl_inf_object.monitor_if, "wlan0", 5);
	}
	LOGDEBUG("connect nl success\n");
	return WMG_STATUS_SUCCESS;
}
