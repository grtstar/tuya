...
static int ap_enable(void *param, void *cb_msg)
{
	...
	if (ap_object.ap_state == WIFI_AP_ENABLE) {
		if (ap_config == NULL) {
			LOGW("wifi ap is already enabled\n");
			*cb_args = WMG_STATUS_SUCCESS;
			return WMG_STATUS_SUCCESS;
		}
	} else {
		LOGD("wifi ap enabling...\n");
		if (ap_config == NULL) {
			LOGE("ap config is NULL, not set ap config\n");
			*cb_args = WMG_STATUS_FAIL;
			return WMG_STATUS_FAIL;
		} else {
			LOGD("ap config is valid, set ap config\n");
		}
	}

	ret = ap_object.platform_inf[PLATFORM_LINUX]->ap_hapd_enable(ap_config);
	if (ret) {
		LOGE("failed to enable ap hapd, when set ap config\n");
		*cb_args = WMG_STATUS_FAIL;
		return WMG_STATUS_FAIL;
	}
...
static int ap_get_config(void *param, void *cb_msg)
{
	...
	if(ap_object.init_flag == WMG_FALSE) {
		LOGE("wifi ap mode has not been initialized\n");
		*cb_args = WMG_STATUS_FAIL;
		return WMG_STATUS_FAIL;
	}
	if(ap_object.ap_enable == WMG_FALSE) {
		LOGW("wifi ap mode already disabled\n");
		*cb_args = WMG_STATUS_FAIL;
		return WMG_STATUS_FAIL;
	}

	if(ap_object.ap_state != WIFI_AP_ENABLE) {
		LOGW("wifi ap has not been enable now\n");
		*cb_args = WMG_STATUS_UNHANDLED;
		return WMG_STATUS_UNHANDLED;
	}

	if(ap_config == NULL) {
		LOGE("ap config is NULL\n");
		*cb_args = WMG_STATUS_FAIL;
		return WMG_STATUS_FAIL;
	}
...
static wmg_status_t ap_mode_enable(int* erro_code)
{
	...
	if (ret) {
		LOGE("failed to connect to hostapd\n");
		return WMG_STATUS_FAIL;
	}
	ap_object.hapd_run = WMG_TRUE;
	ap_object.ap_state = WIFI_AP_ENABLE;
	LOGD("start hapd success\n");
...
static wmg_status_t ap_mode_disable(int* erro_code)
{
	...
	if(ap_object.init_flag == WMG_FALSE) {
		LOGW("wifi ap has not been initialized\n");
		return WMG_STATUS_SUCCESS;
	}
	if (ap_object.ap_enable == WMG_FALSE) {
		LOGW("wifi ap already disabled\n");
...
{
	if(ap_object.init_flag == WMG_FALSE) {
		LOGE("wifi ap has not been initialized\n");
		return WMG_STATUS_FAIL;
	}
	if(ap_object.ap_enable == WMG_FALSE) {
		LOGW("wifi ap already disabled\n");
