...
static const char IFACE_DIR[]           = "/var/run/hostapd";
static char primary_iface[IFACE_VALUE_MAX];
static const char HAPD_CONFIG_FILE[]    = "/data/hostapd.conf";
//static const char CONTROL_IFACE_PATH[]  = "/etc/wifi/sockets";
static const char CONTROL_IFACE_PATH[]  = "/var/run/hostapd";
...
static int wifi_send_command(const char *cmd, char *reply, size_t *reply_len)
{
	int ret;
	if (ctrl_conn == NULL) {
		LOGE("Not connected to hostapd - \"%s\" command dropped.\n", cmd);
		return -1;
	}
...
static int parse_ap_config(char *reply, wifi_ap_config_t *ap_config)
{
	...
	if (pos != NULL) {
		...
		if (strncmp(pos, "ENABLED", 7) != 0) {
			LOGW("Warning: ap state is inactive\n");
			return -1;
		} else {
			LOGD("ap state is enable\n");
			pch = strtok(reply, "'\n'");
			while (pch != NULL) {
				if (strncmp(pch, "ssid[0]=", 8) == 0) {
					if(strlen(pch + 8) > SSID_MAX_LEN) {
						LOGDEBUG("ssid is too long:%d",strlen(pch + 8));
						return -1;
					}
...
static int linux_ap_hapd_set_config(wifi_ap_config_t *ap_config)
{
	...
	if((ap_config == NULL) || (ap_config->ssid == NULL)){
		LOGE("ap config format is error, ssid can't NULL\n");
	...
	if (ret) {
		LOGE("failed to set ssid '%s', reply %s\n",ap_config->ssid, reply);
		return WMG_STATUS_FAIL;
	}
	...
	if (ap_config->channel < 14) {
		...
		ret = command_to_hostapd(cmd, reply, sizeof(reply));
		if (ret) {
			LOGE("failed to set channel '%u', reply %s\n", ap_config->channel, reply);
			return WMG_STATUS_FAIL;
		}
	} else {
	...
	switch (ap_config->sec){
		...
		case WIFI_SEC_WEP:
			...
			ret = command_to_hostapd(cmd, reply, sizeof(reply));
			if (ret){
				LOGE("failed to set wep_key0 '%s', reply %s\n", ap_config->psk, reply);
				return WMG_STATUS_FAIL;
			}
			LOGD("set wep_key0 %s\n", ap_config->psk);
			break;
		case WIFI_SEC_WPA_PSK:
			if (ap_config->psk != NULL) {
				...
				ret = command_to_hostapd(cmd, reply, sizeof(reply));
				if (ret) {
				LOGE("failed to set wpa key mgmt WPA-PSK, reply %s\n", reply);
					return WMG_STATUS_FAIL;
				}
				passwd_len = strlen(ap_config->psk);
				if(passwd_len < 8) {
					LOGE("Password shorter than 8 characters!\n");
					return WMG_STATUS_FAIL;
				}
				sprintf(cmd, "SET rsn_pairwise CCMP");
				ret = command_to_hostapd(cmd, reply, sizeof(reply));
				if (ret) {
					LOGE("failed to set rsn_pairwise CCMP");
					return WMG_STATUS_FAIL;
				}
				sprintf(cmd, "SET wpa_passphrase %s", ap_config->psk);
				ret = command_to_hostapd(cmd, reply, sizeof(reply));
				if (ret) {
					LOGE("failed to set psk '%s', reply %s\n",ap_config->psk, reply);
					return WMG_STATUS_FAIL;
				}
				LOGD("set psk %s\n", ap_config->psk);
			} else {
				LOGE("psk is NULL, but sec not WIFI_SEC_NONE(WIFI_SEC_WPA_PSK)\n");
				LOGE("Please check ap config format\n");
				return WMG_STATUS_FAIL;
			}
			break;
		case WIFI_SEC_WPA2_PSK:
			if (ap_config->psk != NULL) {
				...
				LOGE("psk is NULL, but sec not WIFI_SEC_NONE(WIFI_SEC_WPA2_PSK)\n");
				LOGE("Please check ap config format\n");
				return WMG_STATUS_FAIL;
			}
			break;
		default:
			LOGE("not support sec\n");
			LOGE("Please check ap config format\n");
			return WMG_STATUS_FAIL;
			break;
	}
...
static int linux_ap_hapd_cmd_enable()
{
	...
	ret = command_to_hostapd(cmd, reply, sizeof(reply));
	if (ret)
		LOGE("failed to enable ap, reply %s\n", reply);
...
static int linux_ap_hapd_cmd_disable()
{
	...
	ret = command_to_hostapd(cmd, reply, sizeof(reply));
	if (ret)
		LOGE("failed to disable ap, reply %s\n", reply);
...
static int linux_ap_hapd_cmd_status()
{
	...
	ret = command_to_hostapd(cmd, reply, sizeof(reply));
	if (ret) {
		LOGE("failed to get ap status\n");
		return -1;
	}
	if (strstr(reply, "state=ENABLED") != NULL) {
		LOGD("ap status is: enable\n");
		return 0;
	} else if (strstr(reply, "state=DISABLED") != NULL) {
		LOGD("ap status is: disable\n");
		return 1;
	} else {
...
static int linux_ap_hapd_cmd_reload()
{
	...
	LOGD("reload ap config\n");
	cmd[CMD_MAX_LEN] = '\0';
	strncpy(cmd, "RELOAD", CMD_MAX_LEN);
	ret = command_to_hostapd(cmd, reply, sizeof(reply));
	if (ret)
		LOGE("failed to reload ap config, reply %s\n", reply);
...
static int start_hostapd(int try_times)
{
	...
	for(try_cnt = 0; try_cnt < try_times; try_cnt++, i = 0) {
		...
		if (ret == 0) {
			...
			LOGD("start hostapd times %d success\n", (try_cnt + 1));
			return 0;
		}
	}
	return -1;
...
static int wmg_sta_device_remove(dev_node_t *list_head, char *bssid)
{
	...
	if (ret == -1)
		LOGW("%s is not found, remove device failed\n");
...
static int hapd_dispatch_event(const char *event_str, int nread)
{
	...
	if (!event_str || !event_str[0]) {
		LOGW("hostapd event is NULL!\n");
		return 0;
	}

	LOGD("receive hapd event: '%s'\n", event_str);
...
static int linux_ap_hapd_init(ap_hapd_event_cb_t ap_event_cb)
{
	if(ap_hapd_inf_object.ap_hapd_init_flag == WMG_FALSE) {
		...
		if (ap_hapd_inf_object.dev_list != NULL) {
			...
			LOGE("failed to allocate memory for device list\n");
		...
		if(ap_hapd_inf_object.ap_event_handle != NULL) {
			...
			LOGE("failed to allocate memory for linux hapd event_handle\n");
			goto event_erro;
		}
		if(ap_event_cb != NULL){
		...
		LOGI("linux hostapd already init\n");
	}
	return WMG_STATUS_SUCCESS;
...
static int linux_connect_to_hostapd()
{
	...
	if (check_process_is_exist("wpa_supplicant", 14) == 1) {
		LOGDEBUG("wpa_supplicant is running, need to stop it\n");
		system("pidof wpa_supplicant | xargs kill -9");
		if (check_process_is_exist("wpa_supplicant", 14) == 1) {
			sleep(1);
			if (check_process_is_exist("wpa_supplicant", 14) == 1) {
				LOGE("kill wpa_supplicant failed\n");
				return WMG_STATUS_FAIL;
			}
		}
	}
	...
	if (check_process_is_exist("hostapd", 7) == 0) {
		LOGDEBUG("hostapd is not running, need to start it\n");
		...
		LOGD("start hostapd success\n");
	} else {
		LOGD("hostapd is already running\n");
	}
	...
	if (access(IFACE_DIR, F_OK) == 0) {
		...
		LOGE("hostapd socket interface not exists\n");
		return -1;
	}
	...
	if (ret) {
		LOGE("failed to initialize linux hapd event socket\n");
		ret = WMG_STATUS_FAIL;
	...
	if (ret) {
		LOGW("failed to clear linux hapd event socket\n");
	}

	ret = os_net_thread_create(&ap_hapd_inf_object.ap_pid, NULL, hapd_event_thread, NULL, 0, 4096);
	if (ret) {
		LOGE("failed to create linux ap event handle thread\n");
		ret = WMG_STATUS_FAIL;
		goto socket_err;
	}
	LOGD("create linux ap event handle thread success\n");
...
static int linux_ap_hapd_enable(wifi_ap_config_t *ap_config)
{
	...
	if(ret == 1){
		LOGDEBUG("hapd is already disable\n");
	} else if (ret == 0){
		LOGDEBUG("hapd is already enable, disable now...\n");
		ret = linux_ap_hapd_cmd_disable();
		if(ret){
			LOGE("reload hapd config faile\n");
			return WMG_STATUS_FAIL;
		}
	} else {
		LOGE("failed to get ap hapd status\n");
		return WMG_STATUS_FAIL;
	}

	ret = linux_ap_hapd_set_config(ap_config);
	if(ret){
		LOGE("failed to set ap hapd config\n");
		return WMG_STATUS_FAIL;
	}

	LOGDEBUG("Enable ap hapd now...\n");
...
static int linux_ap_hapd_disable()
{
	...
	ret = linux_ap_hapd_cmd_status();
	if(ret == 1){
		LOGDEBUG("hapd is already disable, not need to disable\n");
		return WMG_STATUS_SUCCESS;
	}

	ret = linux_ap_hapd_cmd_disable();
	if(ret){
		LOGE("failed to disable ap hapd\n");
		return WMG_STATUS_FAIL;
	}
...
static int linux_ap_hapd_get_config(wifi_ap_config_t *ap_config)
{
	...
	if (strstr(reply, "state=DISABLED") != NULL) {
		LOGDEBUG("hapd is disabled, please enable first\n");
		return WMG_STATUS_FAIL;
	}

	ret = parse_ap_config(reply, ap_config);
	if (ret) {
		LOGE("failed to parse ap config\n");
		return WMG_STATUS_FAIL;
	}

	if(global_ap_config.ap_config_enable != true) {
		LOGE("failed to get ap config psk\n");
		return WMG_STATUS_FAIL;
	} else {
	...
	while (p != NULL) {
		...
		LOGD("copy device '%s' to device list\n", ap_config->dev_list[count]);
		p = p->next;
		count++;
		if (count >= STA_MAX_NUM)
		break;
	}
	LOGD("number of connected sta is %d\n", count);
