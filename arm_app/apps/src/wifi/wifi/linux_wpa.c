...
#include <poll.h>
#include <pthread.h>
#include "wifi/wpa_ctrl.h"
#include "wifi/linux_wpa.h"
#include "wifi/udhcpc.h"
...
static const char CONTROL_IFACE_PATH[]  = "/var/run/wpa_supplicant";
...
static int ensure_entropy_file_exists()
{
    ...
    if (destfd < 0) {
        LOGD("Cannot create \"%s\": %s\n", SUPP_ENTROPY_FILE, strerror(errno));
        return -1;
    }

    if (TEMP_FAILURE_RETRY(write(destfd, dummy_key, sizeof(dummy_key))) != sizeof(dummy_key)) {
        LOGE("Error writing \"%s\": %s\n", SUPP_ENTROPY_FILE, strerror(errno));
        close(destfd);
        return -1;
    }
    close(destfd);
...
static int wifi_connect_on_socket_path(const char *path)
{
	...
	LOGE("monitor_conn is NULL!\n");
        wpa_ctrl_close(ctrl_conn);
        ctrl_conn = NULL;
        return -1;
    }
    if (wpa_ctrl_attach(monitor_conn) != 0) {
	LOGE("attach monitor_conn error!\n");
    ...
    LOGD("connect to wpa_supplicant ok!\n");
    return 0;
}
...
static int start_supplicant(int try_times)
{
	...
	for(try_cnt = 0; try_cnt < try_times; try_cnt++) {
		sprintf(cmd, "wpa_supplicant -B -i%s -Dnl80211 -c%s -O%s ",
		...
		if (ret == 0) {
			LOGE("failed to start wpa_supplicant times %d\n", (try_cnt + 1));
		} else {
			LOGDEBUG("start wpa_supplicant times %d success\n", (try_cnt + 1));
			return 0;
		}
	}
	return -1;
...
static int wpa_parse_status_info(char *status, wifi_sta_info_t *sta_info)
{
	...
	if (pos != NULL) {
		...
		if (strncmp(pos, "COMPLETED", 9) != 0) {
			LOGW("Warning: wpa state is inactive\n");
			return -1;
		} else {
			LOGD("wpa state is completed\n");
			pch = strtok(status, "'\n'");
			while (pch != NULL) {
				...
				if (strncmp(pch, "ssid=", 5) == 0) {
					if((strlen(pch) - 5) > SSID_MAX_LEN) {
						LOGDEBUG("ssid is too long:%d", (strlen(pch) - 5));
						return -1;
					}
					strcpy(ssid, (pch + 5));
					LOGDEBUG("%s\n", ssid);
...
static int wpa_conf_is_ap_exist(const char *ssid, wifi_secure_t key_mgmt, char *net_id, int *len)
{
	...
	while ((pssid_start = strstr(ptr, ssid)) != NULL) {
		...
		if (ret) {
			LOGE("do get network %s key_mgmt error!\n", net_id);
			return -1;
		}

		LOGD("GET_NETWORK %s key_mgmt reply %s\n", net_id, key_reply);
		LOGD("key type %s\n", key_type);
...
static int wpa_conf_ssid2netid(const char *ssid, int *net_id)
{
	...
	ret = command_to_supplicant(cmd, reply, sizeof(reply));
	if (ret) {
		LOGE("failed to list networks, reply %s\n", reply);
		return ret;
	}

	if(strlen(reply) < 34){
		LOGI("Here has no entry save\n");
		return -1;
	}
...
static int wpa_conf_get_max_priority()
{
	...
	while(p_n != NULL) {
		...
		if (ret) {
			LOGE("do get network priority error!\n");
			return -1;
		}
...
static int wpa_conf_enable_all_networks()
{
	...
	while (p_n != NULL) {
		...
		ret = command_to_supplicant(cmd, reply, sizeof(reply));
		if (ret) {
			LOGE("do enable network %s error!\n", net_id);
			return -1;
		}
...
static int wpa_conf_remove_maxnetid_network()
{
	...
	LOGD("remove network %s!\n", net_id);
	ret = command_to_supplicant(cmd, reply, sizeof(reply));
	if (ret) {
		LOGE("do remove network %s error!\n", net_id);
		return -1;
	}
...
static int linux_sta_wpa_cmd_disconnect()
{
	...
	ret = command_to_supplicant(cmd, reply, sizeof(reply));
	if (ret)
		LOGE("failed to disconnect ap, reply %s\n", reply);
	return ret;
}
...
static int linux_sta_wpa_cmd_reconnect()
{
	...
	ret = command_to_supplicant(cmd, reply, sizeof(reply));
	if (ret)
		LOGE("failed to reconnect ap, reply %s\n", reply);
	return ret;
}
...
static int linux_sta_wpa_cmd_status()
{
	...
	if (strstr(reply, "wpa_state=COMPLETED") != NULL) {
		...
		LOGD("sta status is: disconnect\n");
		return 1;
	} else if (strstr(reply, "wpa_state=SCANNING") != NULL) {
		LOGD("sta status is: scanning\n");
		return 2;
	} else {
		LOGD("ap status is unknowd\n");
		return -1;
	}
}
...
static int wpas_select_network(wifi_sta_cn_para_t *cn_para, char *netid)
{
	...
	if (ret) {
		LOGE("failed to add network, reply %s\n", netid);
		return WMG_STATUS_FAIL;
	}

	LOGDEBUG("add network id=%s\n", netid);
	sprintf(cmd, "SET_NETWORK %s ssid \"%s\"", netid, cn_para->ssid);
	ret = command_to_supplicant(cmd, reply, sizeof(reply));
	if (ret) {
		LOGE("failed to set network ssid '%s', reply %s\n", cn_para->ssid, reply);
		return WMG_STATUS_FAIL;
	}
	...
	case WIFI_SEC_NONE:
		...
		ret = command_to_supplicant(cmd, reply, sizeof(reply));
		if (ret) {
			LOGE("failed to set network key_mgmt, reply %s\n", reply);
		return WMG_STATUS_FAIL;
		}
		break;
	...
	case WIFI_SEC_WPA2_PSK:
		...
		ret = command_to_supplicant(cmd, reply, sizeof(reply));
		if (ret) {
			LOGE("failed to set network psk, reply %s\n", reply);
			return WMG_STATUS_FAIL;
		}
		break;
	case WIFI_SEC_WEP:
		...
		if((passwd_len == 10) || (passwd_len == 26)) {
			...
			LOGDEBUG("The passwd is HEX format!\n");
		} else if((passwd_len == 5) || (passwd_len == 13)) {
			sprintf(cmd, "SET_NETWORK %s wep_key0 \"%s\"", netid, cn_para->password);
			LOGDEBUG("The passwd is ASCII format!\n");
		} else {
			LOGE("The password does not conform to the specification!\n");
			return WMG_STATUS_FAIL;
		}
		ret = command_to_supplicant(cmd, reply, sizeof(reply));
		if (ret) {
		...
		ret = command_to_supplicant(cmd, reply, sizeof(reply));
		if (ret) {
			LOGE("failed to set network auth_alg OPEN SHARED, reply %s\n", reply);
			return WMG_STATUS_FAIL;
		}
		break;
	default:
		LOGE("unknown key mgmt\n");
		return WMG_STATUS_FAIL;
	}
	...
	ret = command_to_supplicant(cmd, reply, sizeof(reply));
	if (ret) {
		LOGE("failed to select network %s, reply %s\n", netid, reply);
		return WMG_STATUS_FAIL;
	}
...
scan_once:
	...
	if (ret_tmp) {
		if (strncmp(reply, "FAIL-BUSY", 9) == 0) {
			LOGD("wpa_supplicant is scanning internally\n");
			sleep(2);
			goto scan_results;
		}
		ret = WMG_STATUS_FAIL;
...
scan_results:
	...
	if (ret_tmp) {
		LOGE("failed to parse scan results\n");
		ret = WMG_STATUS_FAIL;
	} else {
...
static void try_start_udhcpc()
{
	...
	LOGDEBUG("**************************wait 1 seconds********************************\n");
	sleep(1);
	LOGDEBUG("*************************try start udhcpc*******************************\n");

	wpas_event_notify(WIFI_DHCP_START);
	if((is_ip_exist()) != 4) {
		LOGDEBUG("IPv4 is not exist, need to start_udhcpc\n", DHCP_UPDATE_SECONDS);
		start_udhcpc();
		if((is_ip_exist()) == 4) {
		...
		if(strcmp(private_data->old_ssid, private_data->new_ssid)){
			LOGDEBUG("Old ssid(%s) and new ssid(%s) different, need to start_udhcpc\n",
			...
			if(((now_time) - (private_data->last_time)) > DHCP_UPDATE_SECONDS) {
				LOGDEBUG("It(%s) took more than %d seconds to get IP, need to start_udhcpc\n",
				...
				LOGI("(%s)The time to get IP does not exceed %d(%d) seconds, need not to start_udhcpc\n",
						private_data->old_ssid,DHCP_UPDATE_SECONDS, ((now_time) - (private_data->last_time)));
				wpas_event_notify(WIFI_DHCP_SUCCESS);
			}
		}
	}
	LOGDEBUG("**************************try udhcpc end********************************\n");
}
...
static int wpas_dispatch_event(const char *event_str, int nread)
{
	...
	if (!event_str || !event_str[0]) {
		LOGW("wpa_supplicant event is NULL!\n");
		return 0;
	}

	LOGD("receive wpa event: %s\n", event_str);
	...
	if (nae_end) {
		...
		LOGD("received wpa_supplicant event with empty event nae!\n");
		return 0;
	}
	...
	if (!strcmp(event_nae, "DISCONNECTED")) {
		...
		ret = command_to_supplicant(cmd, reply, sizeof(reply));
		if (ret) {
			LOGE("failed to disconnect from ap, reply %s\n", reply);
		}
...
static void *wpas_event_thread(void *args)
{
	...
	for (;;) {
		...
		if (size > 0) {
			...
			if (ret) {
				LOGD("wpa_supplicant terminated\n");
				break;
			}
		} else {
			continue;
...
static int linux_supplicant_init(sta_wpa_event_cb_t sta_event_cb)
{
	if(sta_wpa_inf_object.sta_wpa_init_flag == WMG_FALSE) {
		LOGI("linux supplicant init now\n");
		...
		if(sta_wpa_inf_object.sta_event_handle != NULL) {
			...
			LOGE("failed to allocate memory for linux wpa event_handle\n");
			return WMG_STATUS_FAIL;
		}
		...
		LOGI("linux supplicant already init\n");
	}
...
static void linux_supplicant_deinit(void)
{
	if(sta_wpa_inf_object.sta_wpa_init_flag == WMG_TRUE) {
		LOGI("linux supplicant deinit now\n");
		...
		LOGDEBUG("linux supplicant already deinit\n");
	}
	return;
}
...
static int linux_connect_to_supplicant()
{
	...
	if (check_process_is_exist("hostapd", 7) == 1) {
		LOGDEBUG("hostapd is running, need to stop it\n");
		system("pidof hostapd | xargs kill -9");
		if (check_process_is_exist("hostapd", 7) == 1) {
			sleep(1);
			if (check_process_is_exist("hostapd", 7) == 1) {
				LOGE("kill hostapd failed\n");
				return WMG_STATUS_FAIL;
			}
		}
	}
	...
	if (check_process_is_exist("wpa_supplicant", 14) == 0) {
		LOGDEBUG("wpa_supplicant is not running, need to start it\n");
		...
		LOGD("start wpa_supplicant success\n");
	} else {
		LOGD("wpa_supplicant is already running\n");
	}
	...
	if (access(IFACE_DIR, F_OK) == 0) {
		...
		LOGE("wpa_supplicant socket interface not exists\n");
		return WMG_STATUS_FAIL;
	}
	...
	if (ret) {
		LOGE("failed to initialize linux sta event socket\n");
		ret = WMG_STATUS_FAIL;
		goto wpa_socket_err;
	}
	...
	if (ret) {
		LOGW("failed to clear linux sta event socket\n");
	}

	ret = os_net_thread_create(&sta_wpa_inf_object.sta_pid, NULL, wpas_event_thread, NULL, 0, 4096);
	if (ret) {
		LOGE("failed to create linux sta event handle thread\n");
		ret = WMG_STATUS_FAIL;
		goto socket_err;
	}
	LOGD("create linux sta event handle thread success\n");

	int status = linux_sta_wpa_cmd_status();
	if(status == 0)
	{
	...
	if(status == 2)
	{
		wpas_event_notify_to_sta_dev(WIFI_SCAN_STARTED);
	}
	if(status == 2)
	{
		wpas_event_notify_to_sta_dev(WIFI_UNKNOWN);
	}
	return WMG_STATUS_SUCCESS;
...
static int linux_supplicant_connect_to_ap(wifi_sta_cn_para_t *cn_para)
{
	...
	if (ret_tmp > 0) {
		LOGD("current bss is exist\n");
		if (ret_tmp == 3) {
			LOGD("wifi is already connected to %s\n", cn_para->ssid);
			wpas_event_notify_to_sta_dev(WIFI_CONNECTED);
			return WMG_STATUS_SUCCESS;
		} else {
...
read_event:
	...
	if (ret == WMG_STATUS_SUCCESS) {
		...
		ret_tmp = command_to_supplicant(cmd, reply, sizeof(reply));
		if (!ret_tmp){
			LOGD("save config to wpa_supplicant.conf success\n");
		}
		else{
			LOGW("failed to save config to wpa_supplicant.conf\n");
		}
	}
	...
	if (ret != WMG_STATUS_SUCCESS) {
		...
		ret_tmp = command_to_supplicant(cmd, reply, sizeof(reply));
		if (!ret_tmp){
			LOGD("remove network %s because of connection failure\n", netid);
		}
		else{
			LOGW("failed to remove network %s\n", netid);
		}
	}
...
static int linux_supplicant_disconnect_to_ap(void)
{
	...
	if(linux_sta_wpa_cmd_status() != 0){
		LOGDEBUG("sta is disconnected, need not to disconnect to ap\n");
		return WMG_STATUS_SUCCESS;
	}
	ret = linux_sta_wpa_cmd_disconnect();
	if (ret) {
		LOGE("failed to disconnect ap\n");
		return ret;
	}
...
static int linux_supplicant_get_info(wifi_sta_info_t *sta_info)
{
	...
	ret = command_to_supplicant(cmd, reply, sizeof(reply));
	if (ret) {
		LOGE("failed to get status of wifi station, reply %s\n", reply);
		return WMG_STATUS_FAIL;
	}
	ret = wpa_parse_status_info(reply, sta_info);
	if(ret) {
		LOGE("failed to parse station info\n");
		return WMG_STATUS_FAIL;
	}
	return WMG_STATUS_SUCCESS;
}
...
static int linux_supplicant_list_networks(wifi_sta_list_t *sta_list)
{
	...
	if(sta_list->list_num > LIST_ENTRY_NUME_MAX) {
		LOGW("Sys only support list %d entry\n", LIST_ENTRY_NUME_MAX);
	...
	LOGDEBUG("sys save num is:%d, list buff num is:%d\n", i, sta_list->list_num);
...
static wmg_sta_wpa_inf_object_t sta_wpa_inf_object = {
	...
	.sta_pid = -1,
	.sta_wpa_auto_reconn = WMG_TRUE,	//anthony WMG_FALSE
	.sta_auth_fail_cnt = 0,
