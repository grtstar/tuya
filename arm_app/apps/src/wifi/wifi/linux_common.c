...
wmg_status_t linux_common_set_mac(const char *ifname, uint8_t *mac_addr)
{
	...
	if(sock_mac == -1) {
		LOGE("down network(%s): create mac socket faile\n", ifname);
	...
	if((ioctl(sock_mac, SIOCSIFFLAGS, &ifr_mac)) < 0) {
		LOGE("down network(%s): mac ioctl error\n", ifname);
		close(sock_mac);
		return WMG_STATUS_FAIL;
	}
	close(sock_mac);
	sock_mac = -1;
	LOGDEBUG("wait 3 second\n");
	sleep(3);

	/***** set mac addr to network *****/
	LOGI("ioctl set %s mac: %02x:%02x:%02x:%02x:%02x:%02x\n",ifname, mac_addr[0], mac_addr[1], mac_addr[2], mac_addr[3], mac_addr[4], mac_addr[5]);
	sock_mac = socket(AF_INET, SOCK_STREAM, 0);
	...
	if((ioctl(sock_mac, SIOCSIFHWADDR, &ifr_mac)) < 0) {
		LOGE("set network(%s): mac ioctl error\n", ifname);
		close(sock_mac);
		return WMG_STATUS_FAIL;
	}
	close(sock_mac);
	...
	if((ioctl(sock_mac, SIOCSIFFLAGS, &ifr_mac)) < 0) {
		LOGE("up network(%s): mac ioctl error\n", ifname);
		close(sock_mac);
		return WMG_STATUS_FAIL;
	}
