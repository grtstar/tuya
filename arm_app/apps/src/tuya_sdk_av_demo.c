    ...
    while (1) {
        /*获取到底层的视频流数据，该接口实际映射的是 tkl_venc_get_frame，需要开发者去适配 (可参考开发者文档)，
           ...
           if (0 == tal_venc_get_frame(0, 0, &frame)) {
                printf("frametype:%d, codectype:%d, width=%d, height=%d, pts:%lld, timestamp:%lld, seq:%d \
                        fragment:%d, seq_error:%d, fragment_is_last:%d\n", frame.frametype, frame.codectype,
                        frame.width, frame.height, frame.pts, frame.timestamp, frame.seq, frame.fragment,
                        frame.seq_error, frame.fragment_is_last);
                printf("frame.used_size:%d\n", frame.used_size);
           ...
           int len = rkcamera_get_frame(rkcamera, frame_buff, 40, &is_key_frame);
           if(len > 0)
           {
                if (s_ring_buffer_handles[E_IPC_STREAM_VIDEO_MAIN] == NULL) { //主视频流 ring buffer 空闲时，打开一个新的会话进行写入操作
                    s_ring_buffer_handles[E_IPC_STREAM_VIDEO_MAIN] = tuya_ipc_ring_buffer_open(0, 0, E_IPC_STREAM_VIDEO_MAIN, E_RBUF_WRITE);
                ...
                if (s_ring_buffer_handles[E_IPC_STREAM_VIDEO_MAIN]) { //新的会话打开之后，将原始视频流数据（来自 tkl 层）放到 ring buffer 中
                    ...
                    frame.pts = (long long)tv.tv_sec * 1000 + tv.tv_usec / 1000;
                    tuya_ipc_ring_buffer_append_data(s_ring_buffer_handles[E_IPC_STREAM_VIDEO_MAIN], frame_buff,
                        len, is_key_frame ? TKL_VIDEO_I_FRAME : TKL_VIDEO_PB_FRAME, frame.pts);
    ...
    while (1) {
        ...
        if (0 <= rksound_record_get_frame(frame_buff, rksound_record_get_frame_size())) {
            ...
            if (s_ring_buffer_handles[E_IPC_STREAM_AUDIO_MAIN]) { //新的会话打开之后，将原始音频流数据放到 ring buffer 中
                ...
                gettimeofday(&tv, NULL);
                frame.pts = (long long)tv.tv_sec * 1000 + tv.tv_usec / 1000;
                tuya_ipc_ring_buffer_append_data(s_ring_buffer_handles[E_IPC_STREAM_AUDIO_MAIN], frame_buff, rksound_record_get_frame_size(),
                TKL_AUDIO_FRAME, frame.pts);
